...
this Node uses flags to synchronize ros msgs.
...
class MinimalSubscriber(Node):
    ...
    ----------
        ...
        self.flag_cloud: flag is false till ros msg is received
        self.flag_frame: flag is false till ros msg is received
        self.point_cloud: point cloud of frame
        self.frame: ZED2 left camera frame

    """

    def __init__(self):
        """Constructor."""
        super().__init__('object_publisher_flags_zed2')
        ...
        self.subscription_frame = self.create_subscription(
            Image,
            '/zed2/zed_node/left/image_rect_color',
            self.listener_callback,
            10)
        ...
        # point cloud of frame
        self.point_cloud = []
        # frame of ZED2 left camera
        self.frame = []
        self.subscription_cloud  # prevent unused variable warning
        self.subscription_frame  # prevent unused variable warning

    def listener_callback(self, msg):
        """
        Call when msg is published.

        :param msg: subscribed msg from topic:
         '/zed2/zed_node/point_cloud/cloud_registered'
          or
         '/zed2/zed_node/point_cloud/cloud_registered'
        """
        # check if frame id is related to point cloud topic
        if msg.header.frame_id == 'zed2_left_camera_frame':
            # point cloud object
            self.point_cloud = ZedCLoud(msg)
            # msg received
            self.flag_cloud = True
        else:
            # frame object
            self.frame = ZedFrame(msg)
            # msg received
            self.flag_frame = True
        # check if msgs are received
        if self.flag_cloud and self.flag_frame:
            # use ObjectList msg; custom msg for detected objects
            objects_list = ObjectList()
            # check if timestamp is the same in point cloud and frame
            if self.frame.get_nanosec() == self.point_cloud.get_nanosec() and\
                    self.frame.get_second() == self.point_cloud.get_second():
                # get frame
                frame = self.frame.get_frame()
                # get point cloud
                point_cloud = self.point_cloud.get_cloud()
                # get predictions
                ...
                for i in range(len(predictions.xyxy[0])):
                    objects_list.detected_objects.append(
                        self.get_object_msg(
                            prediction=predictions.xyxy[0][i],
                            width=frame.shape[1],
                            height=frame.shape[0],
                            point_cloud=point_cloud))
                # publish ObjectList
                self.publisher_.publish(objects_list)
                self.get_logger().info('Publishing Object')
