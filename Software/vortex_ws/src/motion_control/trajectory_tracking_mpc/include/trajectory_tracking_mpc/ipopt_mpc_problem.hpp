...
#ifndef TRAJECTORY_TRACKING_MPC__IPOPT_MPC_PROBLEM_HPP_
#define TRAJECTORY_TRACKING_MPC__IPOPT_MPC_PROBLEM_HPP_
...
public:
  ...
  MPCProblem(
    ...
    const VectorXd & Q, const VectorXd & R1, const VectorXd & R2,
    const Vector12d & desired_state, const Vector12i & state_indices,
    const Vector6i & control_indices, const Vector12i & error_indices)
  {
    AUV_ = AUV;
    T_ = T;
    dt_ = dt;
    ...
    desired_state_ = desired_state.template cast<ADscalar>();
  }
  void operator()(ADvector & fg, const ADvector & vars)
  {
    ...
    for (int i = 0; i < T_; i++) {
      // Error cost
      ADvector error = vars(state_indices_ + Vector12i::Ones() * i) - desired_state_;
...
private:
  ...
  Vector12i error_indices_;
  Vector6i control_indices_;
};
#endif  // TRAJECTORY_TRACKING_MPC__IPOPT_MPC_PROBLEM_HPP_
