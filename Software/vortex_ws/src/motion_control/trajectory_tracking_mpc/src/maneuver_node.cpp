...
/* Constructor */
ManeuverNode::ManeuverNode(const rclcpp::NodeOptions & options)
: Node(options.arguments()[0], options)
{
  ...
  guidance_sub = this->create_subscription<custom_ros_interfaces::msg::MMPC>(
    "/guidance/MMPC", 1, std::bind(&ManeuverNode::GuidanceCB, this, _1));
  ...
  mpc_params["w_ye"] = this->get_parameter("w_ye").as_double();
  mpc_params["w_psie"] = this->get_parameter("w_psie").as_double();
  mpc_params["w_ue"] = this->get_parameter("w_ue").as_double();
  mpc_params["w_ve"] = this->get_parameter("w_ve").as_double();
  mpc_params["w_re"] = this->get_parameter("w_re").as_double();
  ...
  mpc_params["My"] = this->get_parameter("My").as_double();
...
int main(int argc, char ** argv)
{
  ...
  options.arguments({"maneuver_node"});
  auto node = std::make_shared<ManeuverNode>(options);
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
