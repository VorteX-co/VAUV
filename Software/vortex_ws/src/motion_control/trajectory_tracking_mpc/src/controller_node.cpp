...
: Node(options.arguments()[0], options)
{
  ...
  Eigen::VectorXd R2 =
    vector_to_eigen(this->get_parameter("R1").as_double_array());
  int T = this->get_parameter("T").as_int();
  ...
  this->mpc_.set_params(
    mass, volume, T, Ib, r_cob, r_cog, Ma, Dlinear, Dquad, Q,
    R1, R2, tau_max, dt);
  /*****************************************************
...
void Controller::feedbackCallback(const std_msgs::msg::Float32MultiArray::SharedPtr msg)
{
  ...
  Vector6d wrench = mpc_.action(state, desired_state, desired_acc);
