...
#include "../include/depth_mpc/controller_node.hpp"
#include <math.h>
#include <string>
#include <map>
#include <memory>
...
: Node(options.arguments()[0], options)
{
  ...
  guidance_sub = this->create_subscription<custom_ros_interfaces::msg::DMPC>(
    "/guidance/DMPC", 1, std::bind(&ControllerNode::GuidanceCB, this, _1));

  // Publishers
  wrench_pub = this->create_publisher<geometry_msgs::msg::WrenchStamped>(
    "/rexrov/thruster_manager/input_stamped", 10);
  // msg
  ...
  mpc_params["w_thetae"] = this->get_parameter("we_thetae").as_double();
  mpc_params["we_we"] = this->get_parameter("we_we").as_double();
